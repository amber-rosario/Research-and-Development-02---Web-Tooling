...
<div class="container mt-3">
  ...
  ?>
          ...
          <tr>
            ...
            <td>
              <input type="number" name="qty" required min="1" value="<?= $fetch_cart['qty']; ?>" max="99" maxlength="2" class="qty btn rounded-pill border border-warning">
            </td>
            <td><?php echo $sub_total = $fetch_cart['qty'] * $fetch_cart['price'] ;?></td>
            <td>
              <button type="submit" class="btn btn-warning rounded-pill" name="update_cart">Update</button>
              <input type="submit" name="delete_item" value="Delete" class="btn btn-danger rounded-pill" onclick="return confirm('delete this item?');">
            </td>
          </tr>
          </form>
    <?php 
